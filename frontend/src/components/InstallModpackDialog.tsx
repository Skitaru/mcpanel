  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl surface p-0 relative max-h-[90vh] flex flex-col animate-slide-up overflow-hidden rounded-xl">
        ...
        {/* Header */}
        <div className="flex items-center gap-2.5 px-6 py-4 border-b border-edge">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <label className="block">
            ...
            <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)}
              ...
              className="w-full rounded-lg border border-edge bg-void px-3 py-2 text-sm text-white font-mono
          ...
          <div className="relative">
            ...
            <input type="text" value={searchQuery}
              ...
              className="w-full rounded-lg border border-edge bg-void pl-9 pr-3 py-2 text-sm text-white
                         placeholder:text-muted focus:border-accent/40 focus:outline-none disabled:opacity-40" />
          </div>

          {/* Search error */}
          {searchError && <div className="rounded-lg border border-danger/20 bg-danger/5 px-3 py-2.5"><p className="text-xs text-red-400">{searchError}</p></div>}
          ...
          {results.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] text-muted">{results.length} found</span>
              </div>
                ...
                {results.map(p => (
                  <button key={p.id} onClick={() => selectPack(p)}
                    className={`flex items-center gap-2.5 rounded-lg border p-2.5 text-left transition ${
                      selectedPack?.id === p.id ? "border-violet-500/40 bg-accent/10" : "border-edge hover:border-accent/40"
                    }`}>
                    {p.logo?.thumbnailUrl ? (
                      ...
                      <div className="h-10 w-10 rounded bg-void shrink-0 flex items-center justify-center text-[10px] text-slate-600 font-bold">{p.name.slice(0, 2).toUpperCase()}</div>
                    )}
          ...
          {selectedPack && (
            <div className="space-y-3">
              <div className="rounded-lg border border-edge bg-void p-3 flex items-center gap-3">
                {selectedPack.logo?.thumbnailUrl ? (
                  <img src={selectedPack.logo.thumbnailUrl} alt="" className="h-10 w-10 rounded shrink-0" />
                ) : (
                  <div className="h-10 w-10 rounded bg-surface shrink-0 flex items-center justify-center text-[10px] text-slate-500 font-bold">{selectedPack.name.slice(0, 2).toUpperCase()}</div>
                )}
                <div>
              ...
              {loadingFiles ? (
                ...
                <label className="block">
                  ...
                  <select value={selectedFile?.id ?? ""} onChange={e => {
                    ...
                    className="w-full rounded-lg border border-edge bg-void px-3 py-2.5 text-sm text-white
                               focus:border-accent/40 focus:outline-none appearance-none">
                    {files.map(f => (
          ...
          {selectedPack && selectedFile && (
            <div className="space-y-3">
              <div className="w-px-full h-px bg-edge" />
              <label className="block">
                <span className="mb-1.5 block text-[10px] font-semibold text-slate-600 uppercase tracking-wider">Server Name <span className="text-muted">(optional)</span></span>
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder={selectedPack.name}
                  className="w-full rounded-lg border border-edge bg-void px-3 py-2 text-sm text-white
                             placeholder:text-muted focus:border-accent/40 focus:outline-none" />
              </label>
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  ...
                  <select value={ram} onChange={e => setRam(e.target.value)}
                    className="w-full rounded-lg border border-edge bg-void px-3 py-2 text-sm text-white
                               focus:border-accent/40 focus:outline-none appearance-none">
                    {RAM_OPTIONS.map(o => <option key={o} value={o} className="bg-surface">{o}</option>)}
                  </select>
                </label>
                <label className="block">
        ...
        {selectedPack && selectedFile && !installing && (
          <div className="flex items-center justify-between border-t border-edge px-5 py-3">
            ...
            <button onClick={handleInstall}
              className="rounded-lg bg-accent px-5 py-2 text-sm font-medium text-white transition
                         hover:bg-accent-strong flex items-center gap-2">
