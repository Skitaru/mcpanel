...
export default function FileManagerTab({ serverId }: Props) {
  ...
  return (
    <div className="relative flex flex-col lg:flex-row gap-0 overflow-hidden rounded-xl border border-purple-500/15 bg-[#0e0d14] h-[calc(100vh-16rem)] lg:h-[calc(100vh-12rem)]"
      ...
      {/* ── Left: File tree ── */}
      <div className="flex flex-col lg:w-64 lg:shrink-0 border-b lg:border-b-0 lg:border-r border-purple-500/15 min-h-0">
        {/* Toolbar */}
        <div className="flex items-center gap-0.5 border-b border-purple-500/15 px-2 py-1.5">
          <button onClick={() => setShowCreate("file")}
            className="rounded p-1.5 text-slate-500 transition hover:bg-purple-500/5 hover:text-slate-300" title="New File">
            <FilePlus className="h-3.5 w-3.5" />
          </button>
          <button onClick={() => setShowCreate("folder")}
            className="rounded p-1.5 text-slate-500 transition hover:bg-purple-500/5 hover:text-slate-300" title="New Folder">
            <FolderPlus className="h-3.5 w-3.5" />
          </button>
          <button onClick={handleUploadClick} disabled={uploading}
            className="rounded p-1.5 text-slate-500 transition hover:bg-purple-500/5 hover:text-violet-400" title="Upload Files">
        ...
        {/* Breadcrumbs */}
        <div className="flex items-center gap-0.5 overflow-x-auto border-b border-purple-500/15 px-3 py-1.5 text-[11px]">
          {breadcrumbs.map((crumb, i) => (
            <span key={crumb.path} className="flex items-center gap-0.5 whitespace-nowrap">
              ...
              <button onClick={() => { setSelectedFile(null); setFileContent(""); setCurrentPath(crumb.path); }}
                className="rounded px-1 py-0.5 text-slate-500 transition hover:bg-purple-500/5 hover:text-slate-300">
                {crumb.label}
              </button>
            </span>
          ))}
        ...
        {/* Create form */}
        {showCreate && (
          <form onSubmit={e => { e.preventDefault(); handleCreate(); }} className="flex items-center gap-1.5 border-b border-purple-500/15 px-2 py-1.5">
            ...
            <input type="text" value={createName} onChange={e => setCreateName(e.target.value)}
              ...
              className="flex-1 rounded border border-purple-500/15 bg-[#0a0c10] px-1.5 py-0.5 text-[11px] text-slate-200
        ...
        {/* Drag overlay */}
        {dragOver && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-[#0e0d14]/90 backdrop-blur-sm pointer-events-none">
          ...
          {loading ? (
            ...
            <ul>
              {currentPath !== "/" && (
                <li>
                  <button onClick={goUp}
                    className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs text-slate-400 transition hover:bg-purple-500/5">
                ...
                return (
                  <li key={f.name} className="group relative">
                    <button onClick={() => {
                      ...
                      className={`flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs transition hover:bg-purple-500/5 ${isSelected ? "bg-violet-500/10 text-violet-300" : bin ? "text-slate-700 cursor-default" : "text-slate-300"}`}>
                    ...
                    {deleteTarget === f.name ? (
                      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1 rounded bg-[#0e0d14] border border-purple-500/15 px-1.5 py-0.5">
      ...
      <div className="flex flex-1 flex-col min-w-0 min-h-0">
        {/* Editor header */}
        <div className="flex items-center justify-between border-b border-purple-500/15 px-3 py-1.5 gap-2">
          <span className="truncate font-mono text-[11px] text-slate-500 min-w-0">{selectedFile ?? "No file selected"}</span>
          {selectedFile && (
            <div className="flex items-center gap-1 shrink-0">
              {/* Search bar */}
              {searchOpen ? (
                <div className="flex items-center gap-1 rounded-md border border-purple-500/15 bg-[#0a0c10] px-2 py-0.5">
                ...
                <button onClick={() => setSearchOpen(true)}
                  className="rounded p-1 text-slate-500 transition hover:bg-purple-500/5 hover:text-slate-300"
                  title="Find in file">
              ...
              <button onClick={downloadFile} className="rounded p-1 text-slate-500 transition hover:bg-purple-500/5 hover:text-slate-300" title="Download">
