              ...
              <header className="mb-8 flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-1.5 flex items-center gap-3 text-sm text-slate-500">
                    <span className="flex items-center gap-1.5"><span className={`inline-block h-2 w-2 rounded-full ${statusColor(server.status)}`} />{statusLabel(server.status)}</span>
                ...
                <div className="flex items-center gap-2">
                  {server.status === "running" ? (
                    <>
                      <button disabled={acting} onClick={() => handleAction("restart")} className="flex items-center gap-1.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-400 transition hover:border-amber-500/50 hover:bg-amber-500/20 disabled:opacity-50"><RefreshCw className="h-3.5 w-3.5" /> Restart</button>
                      <button disabled={acting} onClick={() => handleAction("stop")} className="flex items-center gap-1.5 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-400 transition hover:border-red-500/50 hover:bg-red-500/20 disabled:opacity-50"><Square className="h-3.5 w-3.5" /> Stop</button>
                    </>
                  ) : (
                  ...
                  <button onClick={() => setEditOpen(true)} className="flex items-center gap-1.5 rounded-lg border border-white/[0.04] px-3 py-2 text-xs text-neutral-400 transition hover:border-white/[0.08] hover:text-neutral-200"><Settings className="h-3.5 w-3.5" /> Edit</button>
                  {deleteConfirm ? (
                    <div className="flex items-center gap-1.5 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2"><span className="text-xs text-red-400">Delete?</span><button onClick={handleDelete} disabled={deleting} className="rounded bg-red-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-red-500 disabled:opacity-50">{deleting?"…":"Yes"}</button><button onClick={()=>setDeleteConfirm(false)} disabled={deleting} className="rounded bg-slate-700 px-2.5 py-1 text-xs text-slate-300 hover:bg-slate-600">No</button></div>
                  ) : (
